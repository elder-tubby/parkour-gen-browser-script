...
class TypeDropdown extends Dropdown {
    ...
    toggleVisibility() {
        ...
        if (listOfGroups) {
            ...
            this.createGroupDropdown(selectedState.type);
        }
    ...
    createGroupDropdown() {
        ...
        const listOfGroups = mapsStructureData[firstType];  // Get the groups data for the first type
        console.log("map groups in create groupdropdow: ", listOfGroups);
        if (!listOfGroups) return;

        if (!this.groupsDropdown) {
            this.groupsDropdown = new GroupDropdown(this.container, listOfGroups);

        } else {
            this.groupsDropdown.setOptions(Object.keys(listOfGroups));
...
class GroupDropdown extends Dropdown {
    ...
    handleChange(event) {
        ...
        this.createMapDropdown(selectedState.group);
        selectedState.mapId = null;
        disableMapRelatedButtons(true);
    }
